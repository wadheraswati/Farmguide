...
- (void)setup {
    ...
    [self bringSubviewToFront:self.leftViewContainer];
}
...
- (void)setupLeftViewContainer {
    ...
    NSLayoutConstraint *widthConstraint = [NSLayoutConstraint constraintWithItem:self.leftViewContainer attribute:NSLayoutAttributeWidth relatedBy:NSLayoutRelationEqual
                                                                          toItem:nil attribute:NSLayoutAttributeNotAnAttribute multiplier:2 constant:kJVDefaultViewContainerWidth];
    NSArray *constraints = @[
...
- (void)applyShadowToCenterViewContainer {
    ...
    UIView *blackOverlay = [[UIView alloc] initWithFrame:CGRectMake(0, 0, self.centerViewContainer.bounds.size.width, self.centerViewContainer.bounds.size.height)];
    blackOverlay.backgroundColor = [[UIColor blackColor] colorWithAlphaComponent:0.75];
    blackOverlay.tag = 2346;
    blackOverlay.alpha = 0;
    [self.centerViewContainer addSubview:blackOverlay];
    [self.centerViewContainer bringSubviewToFront:blackOverlay];
...
- (void)removeShadowFromCenterViewContainer {
    ...
    [UIView animateWithDuration:0.25 animations:^{
        [self.centerViewContainer viewWithTag:2346].alpha = 0;
    } completion:^(BOOL finished) {
        [[self.centerViewContainer viewWithTag:2346] removeFromSuperview];
    }];
}
