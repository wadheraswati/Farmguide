...
+ (UIImage *)mergeBackImage:(UIImage *)backImage withFrontImage:(UIImage *)frontImage
{
    ...
    CGRect rect = CGRectMake(0, 0, backImage.size.height, backImage.size.height);
...
+ (UIImage *)mergeBackImage:(UIImage *)backImage withFrontImageScaled:(UIImage *)frontImage
{
    ...
    CGRect rect2 = CGRectMake(backImage.size.width*0.075, backImage.size.height*0.075, backImage.size.width*0.85, backImage.size.height*0.85);
...
- (UIImage *)ApplyVignetteToImage:(UIImage *)image {
    ...
    [vignette setValue:@0.5 forKey:@"inputRadius"];
    [vignette setValue:@0.5 forKey:@"inputIntensity"];
    [vignette setValue:coreImage forKey:@"inputImage"];
    
    return [UIImage imageWithCIImage:vignette.outputImage];
}
- (UIImage *)imageFromString:(NSString *)string attributes:(NSDictionary *)attributes size:(CGSize)size {
    UIGraphicsBeginImageContextWithOptions(size, NO, [UIScreen mainScreen].scale);
    CGSize stringBoundingBox = [string sizeWithAttributes:@{NSFontAttributeName:attributes[NSFontAttributeName]}];
    [string drawInRect:CGRectMake(0, (size.height/2) - (stringBoundingBox.height/2), size.width, size.height) withAttributes:attributes];
    UIImage *image = UIGraphicsGetImageFromCurrentImageContext();
    UIGraphicsEndImageContext();
